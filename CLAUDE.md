...
| `comedybar` | ✅ **built & verified** | `scrapers/comedybar.py`. Comedy Bar sells via SmartTicket at **tickets.comedybar.net** — clean `.show_cube` cards: `.h2` title, `.show_date` ("ביום חמישי, 25 ביוני 2026" — explicit year, Hebrew month), `.show_time`, `.theater_name` venue, `?id=<n>` per date. **Artists only:** recurring club nights (open-mic, marathons, "ערב סטנד אפ עם כוכבי…", "קומדי בר <city>", "במה פתוחה…") are skipped by keyword (`_SKIP`); kept titles are single-comedian "<name> במופע סטנד אפ" → suffix stripped → `clean_artist`. Arabic-script duplicate of a Hebrew act is skipped. **One Show per date**, url `…/#<iso>` (since `show_id` strips the `?query` and the path repeats per date). **Forced `standup`** like COMY. curl_cffi impersonation. Verified **38 date-shows / 8 artists** on 2026-06-21. |
