...
| `comy` | ✅ **built & verified** | `scrapers/comy.py`. COMY (comy.co.il) — **stand-up only**. Homepage `.event` cards: `a.event-inner[href]` (event url), `<h3>` name, `<h5>` date range "DD/MM/YYYY-DD/MM/YYYY". Touring shows under ONE url → **one Show per event** (one alert per new tour, not per date); `date_iso` = the tour's END date so an in-progress tour isn't pruned early; `venue=""`. curl_cffi `impersonate="chrome"`. **Every COMY artist is forced to category `standup`** — `scan.force_comy_standup()` tags `cat_v`/category directly (skips Gemini) and `make_artist_page` also forces standup+is_artist for any `comy`-sourced artist. Verified **89 shows / 85 artists** on 2026-06-21. |
| (others) | not planned | **Cross-source dedup deferred:** a live check found 0 same-artist+same-date overlaps across barby/eventim/gray/kupat. COMY acts *can* also appear on Kupat → a follower might get one alert per source for the same tour; revisit dedup only if this proves noisy. Kupat's other linked sites (brennerock/amphi) are still NOT scraped. |
